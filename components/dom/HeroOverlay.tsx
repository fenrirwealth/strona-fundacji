...
export default function HeroOverlay(): ReactElement {
  return <section className="cinematic-hero relative flex min-h-[calc(100vh-91px)] items-center overflow-hidden px-6 py-24 sm:px-10 lg:px-16 xl:px-24" aria-labelledby="hero-title">
    ...
    <div className="relative z-10 mx-auto w-full max-w-7xl">
      <p className="mb-7 flex items-center gap-3 text-xs font-semibold uppercase tracking-[.28em] text-gold sm:text-sm"><span className="h-px w-12 bg-gold" aria-hidden="true" />Każdy gest ma siłę · Fundacja Lepszy Dom Lepsze Jutro</p>
      <h1 id="hero-title" className="max-w-5xl font-display text-[clamp(3.5rem,9vw,8.7rem)] font-normal leading-[.86] tracking-[-.055em] text-cream">
        {words.map(word => <span key={word} className="mr-[.22em] inline-block last:mr-0">{word}</span>)}
      </h1>
      <p className="mt-8 max-w-xl text-base leading-8 text-white/75 sm:text-lg">Pomagamy dzieciom i rodzinom odzyskać bezpieczeństwo, sprawczość i wiarę, że dobra przyszłość naprawdę jest możliwa.</p>
      <div className="mt-10 flex flex-col gap-4 sm:flex-row">
        <a href="#wsparcie" className="cinematic-cta group inline-flex min-h-14 items-center justify-center gap-3 rounded-full bg-gold px-8 py-4 text-sm font-bold uppercase tracking-[.16em] text-night transition duration-300">Chcę pomóc <span className="text-xl transition-transform duration-300 group-hover:translate-x-1" aria-hidden="true">→</span></a>
